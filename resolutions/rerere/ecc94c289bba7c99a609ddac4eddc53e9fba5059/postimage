  ...
  const changedSettingLabels = useMemo(
    () => [
      ...
      settings.sidebarV2LargeIcons !==
        DEFAULT_UNIFIED_SETTINGS.sidebarV2LargeIcons
        ? ["Large project icons"]
        : []),
      ...(ownership === "client" &&
      ...
      settings.enableProviderUpdateChecks !==
        ...
        : []),
      ...(ownership === "environment" && isBackgroundActivityDirty
        ? ["Background activity"]
        : []),
      ...(ownership === "environment" &&
...
function ClientLegacyFeaturesSection() {
  ...
  useEffect(() => {
    if (
      searchTargetId === "legacy-plan-mode" ||
      searchTargetId === "legacy-sidebar"
    ) {
      setOpen(true);
    }
  }, [searchTargetId]);
...
function OwnershipSettingsPanel({
  ownership,
}: {
  ownership: SettingsOwnership;
}) {
              ...
              resetAction={
                settings.sidebarV2LargeIcons ? (
                  <SettingResetButton
                    ...
                    onClick={() =>
                      updateSettings({ sidebarV2LargeIcons: false })
                    }
                  />
                ) : null
              }
            ...
            <SettingsRow
              title={
                <span className="inline-flex items-center gap-1.5">
                  Background activity
                  <PolicyTooltip>
                    This shared policy gates background work such as Git
                    refreshes and provider health probes after their individual
                    intervals elapse.
                  </PolicyTooltip>
                </span>
              }
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      ...
                      if (
                        ...
                        updateSettings(
                          backgroundActivityProfileSettings(value),
                        );
                      }
                    }}
                  >
